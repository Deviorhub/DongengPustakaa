...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let res = await fetch("https://backend-dongeng-pustaka.vercel.app:4000/ceritas");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let res = await fetch("http://47.129.5.207:4000/kategoris");
        let { data } = await res.json();
        setKategoris(data);
      } catch (error) {
        console.log(error);
  ...
  useEffect(() => {
    const apiUrl = "http://47.129.5.207:4000/search";
